...
function ExerciseRow({ ex, onPatch, onDelete }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="rounded-xl bg-forge-bg border border-forge-border overflow-hidden">
      <div className="flex items-center gap-2 px-3 py-3">
        <button {...attributes} {...listeners} className="text-forge-muted hover:text-forge-ink cursor-grab active:cursor-grabbing p-1">
          <GripVertical size={16} />
        </button>
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-forge-ink truncate">{ex.name}</div>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <span className="font-mono text-xs text-forge-muted">{ex.sets}×{ex.reps}</span>
          <button onClick={() => setOpen((v) => !v)} className="p-1.5 rounded-lg text-forge-muted hover:text-forge-ink transition-colors">
            {open ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
          </button>
          <button onClick={onDelete} className="p-1.5 rounded-lg text-forge-muted hover:text-forge-red hover:bg-red-500/10 transition-all">
            <Trash2 size={14} />
          </button>
        </div>
      </div>
      ...
      {open && (
        <div className="px-4 pb-4 pt-1 border-t border-forge-border/50 grid grid-cols-2 gap-3">
          <label className="flex flex-col gap-1">
            <span className="text-xs text-forge-muted font-medium">Sätze</span>
            <input type="number" min="1" value={ex.sets}
              onChange={(e) => onPatch("sets", Number(e.target.value))}
              className="px-3 py-2 rounded-lg bg-forge-panel border border-forge-border text-forge-ink text-sm font-mono focus:outline-none focus:border-forge-accent" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-xs text-forge-muted font-medium">Wdh</span>
            <input value={ex.reps}
          ...
          <label className="flex flex-col gap-1">
            <span className="text-xs text-forge-muted font-medium">Pause (s)</span>
            <input type="number" min="0" value={ex.rest_seconds}
              onChange={(e) => onPatch("rest_seconds", Number(e.target.value))}
              className="px-3 py-2 rounded-lg bg-forge-panel border border-forge-border text-forge-ink text-sm font-mono focus:outline-none focus:border-forge-accent" />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-xs text-forge-muted font-medium">Gewicht</span>
            <select value={ex.weight_type}
              onChange={(e) => onPatch("weight_type", e.target.value)}
              className="px-3 py-2 rounded-lg bg-forge-panel border border-forge-border text-forge-ink text-sm focus:outline-none focus:border-forge-accent">
              <option>kg</option>
              <option>%1RM</option>
          ...
          <label className="flex flex-col gap-1">
            <span className="text-xs text-forge-muted font-medium">Effort</span>
          ...
          <label className="flex flex-col gap-1">
            <span className="text-xs text-forge-muted font-medium">RIR</span>
          ...
          <label className="flex flex-col gap-1">
            <span className="text-xs text-forge-muted font-medium">Tempo</span>
          ...
          <label className="flex items-center gap-2 col-span-1 pt-4">
            <input type="checkbox" checked={!!ex.drop_set}
              ...
              className="rounded" />
            <span className="text-sm text-forge-muted">Drop Set</span>
          </label>
          <label className="flex flex-col gap-1 col-span-2">
            <span className="text-xs text-forge-muted font-medium">Notiz</span>
...
export default function WorkoutBuilder({ workoutId, onBack }) {
  ...
  async function addExercise(ex) {
    await api.post(`/workouts/${workoutId}/exercises`, { exercise_id: ex.id });
    load();
  }
  ...
  if (!workout) return <div className="flex items-center justify-center h-screen text-forge-muted text-sm">Lädt…</div>;

  return (
    <div className="max-w-2xl mx-auto px-4 py-6">
      {/* Header */}
      <div className="flex items-center gap-3 mb-6">
        <button onClick={onBack} className="p-2 rounded-xl text-forge-muted hover:text-forge-ink hover:bg-forge-panel transition-all">
          <ArrowLeft size={20} />
        </button>
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold tracking-tight truncate">{workout.name}</h1>
          {workout.goal && <p className="text-sm text-forge-muted">{workout.goal}</p>}
        </div>
        {hasDirty && (
          <button onClick={savePatches} disabled={saving}
            className="px-4 py-2 rounded-xl bg-forge-accent text-white text-sm font-semibold hover:bg-blue-600 disabled:opacity-50 transition-colors">
            {saving ? "…" : "Speichern"}
          </button>
        )}
      </div>
      ...
      {/* Exercise List */}
      {exercises.length === 0 ? (
        <div className="text-center py-14 text-forge-muted text-sm">
