      ...
      <div className={`p-8 flex flex-col h-full ${!pinned ? 'items-center' : ''}`}>
        <div className="flex items-center gap-4 mb-12 relative">
          ...
          <button 
            ...
            className={`absolute top-2 w-8 h-8 rounded-full bg-[var(--card)] border border-[var(--line)] flex items-center justify-center text-[var(--dim)] hover:text-[var(--accent)] transition-all z-10 shadow-lg hover:scale-110 active:scale-90 ${pinned ? '-right-11' : '-right-4 translate-x-full'}`}
          >
        ...
        <nav className="space-y-2 flex-1">
          {NAV_ITEMS.map(({ id, label, Icon }) => {
            ...
            let metric = null;
            if (id === 'habits' && pinned) metric = <span className="ml-auto text-[10px] font-black bg-[var(--accent)]/20 text-[var(--accent)] px-2 py-0.5 rounded-md">80%</span>;
            if (id === 'journal' && pinned) metric = <span className="ml-auto w-2 h-2 rounded-full bg-[var(--accent)] animate-pulse" />;
          ...
          {(isLocalMode() || user?.email?.includes('alpha')) && (
            <button onClick={() => navigate('coach')} title={!pinned ? 'Coach' : ''}
              className={`w-full flex items-center transition-all duration-300 mt-8 ${pinned ? 'gap-4 px-5 py-4 rounded-2xl' : 'justify-center p-4 rounded-2xl'} ${tab === 'coach' ? 'bg-red-500 text-white shadow-xl shadow-red-500/20 font-black scale-[1.02]' : 'text-[var(--dim)] hover:bg-red-500/10 font-bold'}`}>
