...
export default function HabitJournalModal({ isOpen, onClose, habit, date }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-200">
      ...
      <div className="relative w-full max-w-xl bg-fit-card rounded-[32px] border border-fit-line shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6">
          <textarea
            ...
            className="w-full h-48 bg-fit-bg2 border border-fit-line rounded-2xl p-4 text-sm font-medium focus:border-fit-accent outline-none resize-none transition-all text-fit-ink"
            autoFocus
          />
        </div>

        <div className="p-6 border-t border-fit-line/50 bg-fit-bg2/50 flex justify-end gap-3">
          <button 
            onClick={onClose}
            className="btn bg-fit-card border border-fit-line text-fit-ink px-6 py-2.5 text-[10px] font-black uppercase tracking-widest hover:border-fit-accent transition-all"
          >
            Abbrechen
          </button>
          <button 
            onClick={handleSave}
            disabled={saving}
            className="flex items-center gap-2 px-8 py-2.5 bg-fit-accent text-black rounded-xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-fit-accent/20 hover:scale-105 transition-all active:scale-95 disabled:opacity-50"
          >
