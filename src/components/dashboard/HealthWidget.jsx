...
export default function HealthWidget({ days = 14 }) {
  ...
  if (loading) return <div className="animate-pulse h-32 bg-bg2 rounded-2xl" />
  ...
  return (
    <div className="card p-6 shadow-xl bg-gradient-to-br from-card to-bg2 border-accent/10">
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Weight / Scale */}
        <div className="bg-bg2 p-4 rounded-xl border border-line relative overflow-hidden group">
          <div className="flex items-center gap-2 mb-2 text-dim">
            <Scale size={14} />
            <span className="text-[10px] font-black uppercase tracking-widest">Gewicht</span>
          </div>
          <div className="flex items-baseline gap-1">
            <span className="text-xl font-black text-ink">{latest.weight_kg || '—'}</span>
            <span className="text-xs font-bold text-dim">kg</span>
          </div>
          {weights.length > 1 && (
        ...
        <div className="bg-bg2 p-4 rounded-xl border border-line">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-xl font-black text-ink">{latest.sleep_h || '—'}</span>
            <span className="text-xs font-bold text-dim">h</span>
          </div>
          {latest.sleep_score && (
            <div className="text-[9px] font-black text-accent mt-1 tracking-widest uppercase">
        ...
        <div className="bg-bg2 p-4 rounded-xl border border-line">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-xl font-black text-ink">{latest.steps ? (latest.steps / 1000).toFixed(1) : '—'}</span>
            <span className="text-xs font-bold text-dim">k</span>
          </div>
        </div>
        ...
        <div className="bg-bg2 p-4 rounded-xl border border-line">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-xl font-black text-ink">{latest.resting_hr || '—'}</span>
            <span className="text-xs font-bold text-dim">bpm</span>
          </div>
        </div>
      </div>
    </div>
