...
export default function SessionStatus({ plan, todaySession, recent, today, onNavigate }) {
  ...
  return (
    <div className="space-y-8">
      {/* Plan-Hint */}
      {plan?.today && (
        <div className="alpha-card p-8 bg-gradient-to-b from-[var(--card)] to-[var(--bg2)] border-accent/20">
          <div className="flex items-center justify-between gap-3 mb-8">
            <div className="flex items-center gap-2">
              <Zap size={18} className="text-accent" />
              <span className="label-caps !mb-0">Vorschlag</span>
            </div>
            <span className="text-[10px] font-black text-accent bg-accent/10 px-3 py-1 rounded-full uppercase tracking-[0.2em]">Plan</span>
          </div>
          <div className="mb-6">
            <div className="text-3xl font-black text-ink mb-2" style={{ color: blockColor(plan.today.block) || 'var(--accent)' }}>
              {plan.today.block}
            </div>
            <div className="h-1.5 w-16 bg-accent rounded-full shadow-lg shadow-accent/20" />
          </div>
          {planExercises.length > 0 && (
            <div className="flex flex-wrap gap-2">
              {planExercises.slice(0, 6).map((e, i) => (
                <span key={i} className="text-[10px] font-bold px-3 py-1.5 rounded-xl bg-bg2 text-muted border border-line">
      ...
      { (todayExercises.length > 0 || todaySession?.activity) ? (
        <div className="alpha-card p-8 border-accent/30 shadow-2xl shadow-accent/5">
          <div className="label-caps mb-8 flex items-center justify-between">
            <span>Aktuelle Session</span>
            <span className="text-accent font-black tracking-widest">{todaySession?.activity ? ACTIVITY_LABELS[todaySession.activity.type] : todaySession?.block}</span>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex gap-10">
              ...
              {todaySession?.effort && (
                <div className="flex flex-col">
                  <span className="text-3xl font-black text-ink">{todaySession.effort}</span>
                  <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-30">Effort</span>
                </div>
              )}
            </div>
            <button onClick={() => onNavigate?.("session")} className="w-16 h-16 rounded-3xl bg-accent flex items-center justify-center text-black shadow-2xl shadow-accent/30 transition-all hover:scale-110 active:scale-90">
            ...
            {recentSessions.filter(s => s?.date !== today).slice(0, 3).map((s, idx) => {
              ...
              return (
                <button key={s.date || idx} onClick={() => onNavigate?.("session", s.date)}
                  className="w-full text-left px-6 py-4 rounded-3xl bg-card border border-line cursor-pointer hover:border-accent/40 transition-all group hover:bg-accent/5">
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-md font-black text-ink group-hover:text-accent transition-colors">{label}</div>
                      </div>
                    </div>
                    <div className="text-right">
                      {isActivity ? (
                        <div className="flex items-center gap-1.5 text-[11px] font-black text-muted">
