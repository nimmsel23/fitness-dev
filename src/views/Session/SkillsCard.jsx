...
function SkillListScreen({ progress, onOpen }) {
  function SkillRow(skill) {
    const stage = progress[skill.id] ?? 0;
...
function SkillDetailScreen({ skill, stage, onBack, onMaster }) {
...
export default function SkillsCard() {
  ...
  function master(skillId, maxStage) {
    const current = progress[skillId] ?? 0;
    if (current >= maxStage) return;
    persist({ ...progress, [skillId]: current + 1 });
  }
  ...
  return (
    ...
    >
      ...
      {openSkill ? (
        <SkillDetailScreen
          skill={openSkill}
          stage={progress[openSkill.id] ?? 0}
          onBack={() => setOpenSkillId(null)}
          onMaster={() => master(openSkill.id, openSkill.progressions.length - 1)}
        />
      ) : (
