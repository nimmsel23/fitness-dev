              ...
              {ex.setsArray.map((set, sIdx) => (
                  <div key={sIdx} className="grid grid-cols-[1fr_auto_1fr_32px] items-center gap-2 animate-in slide-in-from-left-2 duration-200">
                      <input type="text" inputMode="numeric" placeholder="Reps" value={set.reps || ''} onChange={e => handleRepsChange(e.target.value, sIdx)} 
                        className="text-center font-mono font-black py-3 rounded-2xl bg-fit-bg2 border border-fit-line w-full text-sm focus:border-accent focus:ring-4 focus:ring-accent/5 outline-none transition-all" />
                      
                      <span className="text-fit-dim/30 font-black text-xs italic">@</span>
                      ...
                      <button onClick={() => removeSet(i, sIdx)} className="w-8 h-8 rounded-lg flex items-center justify-center text-fit-dim/30 hover:text-red hover:bg-red/5 transition-all">
                        <X size={14} />
                      </button>
                  </div>
              ))}
