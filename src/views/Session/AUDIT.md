...
- `SkillsCard.jsx` (2026-08-12, Nutzer-Vorgabe): `reps()`/`hold()` haben ein neues `module`-Feld (`module_1_basics`/`module_2_biomechanics`/`module_3_execution`), befüllt für Planche/Front-Lever/Pistol-Squat-Stufen, die in der Vorgabe explizit genannt waren (Rest bleibt `null`). Reines Metadaten-Feld, nirgends im UI/State ausgewertet — kein Coach-Approval, kein Video-Verification, kein Protokoll-Flow. Front-Lever-Stufen Tuck/Advanced Tuck/Straddle bekamen zusätzlich konkrete sets/seconds aus derselben Vorgabe (ersetzen die generischen 3×15s-Defaults). `rest_seconds` aus der Vorgabe (90/120/180s pro Stufe) wurde bewusst NICHT übernommen — `WorkoutRunner` vergibt Pausen pro Rolle (Primär 150s/Sekundär 75s/Conditioning 45s), nicht pro Stufe; dieser Konflikt ist ungelöst liegen gelassen, war nicht Teil des freigegebenen Scopes.
