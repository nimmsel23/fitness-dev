...
export default function InboxCard({ ex, actioning, onApprove, onDelete, onReenrich, onInspect, showUserId = false, asMessage = false }) {
  ...
  return (
    <div className="card cc-dossier p-4 flex flex-col">
      ...
      <div className="flex items-start justify-between gap-2 mb-2.5">
        <h3 className="text-[13.5px] font-bold uppercase tracking-wide text-fit-ink leading-snug">{data.display_name || data.name}</h3>
        {data.category && <span className="cc-tag shrink-0">{data.category}</span>}
      </div>
      ...
      {warnings.length > 0 && (
        <div className="mb-2.5 p-3 bg-fit-red/5 border border-fit-red/10 rounded-xl space-y-1">
          {warnings.map((w, i) => (
      ...
      <p className="text-xs text-fit-dim line-clamp-2 leading-relaxed opacity-70 italic flex-1">
      ...
      <div className="cc-action-row">
        ...
        {onApprove && (
          <button
            ...
            disabled={busy}
            className="cc-btn-ghost go"
          >
            {busy ? 'Warte…' : <><CheckCircle2 size={13} /> Freigeben</>}
          </button>
        )}
      </div>
    </div>
