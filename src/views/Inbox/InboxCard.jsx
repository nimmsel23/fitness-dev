        ...
        <div className="flex items-center flex-wrap gap-2 mb-2">
          ...
          {!showUserId && (
            isProactive
              ...
              : <span className="text-[9px] font-black px-2 py-0.5 bg-green/10 text-green rounded-full border border-green/20 uppercase tracking-tighter">Klient</span>
          )}
        </div>

        <div className="flex flex-wrap gap-1.5 mb-3">
          {data.primary_muscles?.map(m => (
            <span key={m} className="text-[9px] font-bold px-2 py-0.5 bg-accent/5 text-accent rounded-md border border-accent/10">{m}</span>
          ))}
        </div>

        {warnings.length > 0 && (
          <div className="mb-3 p-3 bg-red/5 border border-red/10 rounded-xl space-y-1">
            {warnings.map((w, i) => (
