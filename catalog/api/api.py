"""
=============================================================================
  CATALOG API BACKEND (catalog/api.py)
=============================================================================
  ...
  Server-Rollen:
    ...
    catalog/server.py   :9120  aiohttp Agent-API (wird archiviert sobald verifiziert)
  ...
  - Direktimports aus catalog + anatomy_kb (kein HTTP-Proxy)
=============================================================================
"""
...
from catalog.core.resolver import build_exercise_index, resolve_query, find_by_id
from catalog.agent.teaching import find_lesson
from catalog.planner import build_plan
from catalog.weekly import build_weekly_coverage, resolve_week_selector
...
async def muscles_list(request: Request):
    ...
    try:
        from catalog.core.loader import load_catalog_yaml
...
def muscles_viz():
    ...
    try:
        from catalog.core.loader import load_catalog_yaml
        from catalog.core.yaml_utils import load_yaml
        from catalog.core.paths import DATA_DIR as _FA_DATA
...
async def fitness_export(kind: str, request: Request):
    ...
    if kind == "session":
        # session export via catalog session markdown
        session = data.get("session") or data
        try:
            from catalog.agent.obsidian import export_session_note
