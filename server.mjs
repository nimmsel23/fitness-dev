...
// ── Geo: Standort → nächstes Gym / Adresse / Koordinaten ───────────────────────
// Kein API-Key nötig: Overpass (OSM-POIs) für Gym-Erkennung, Nominatim als
// Adress-Fallback. Beide verlangen einen identifizierenden User-Agent statt
// eines Keys (Usage Policy), kein Tracking/Billing-Risiko bei Low-Volume.
const GEO_USER_AGENT = "fitness-dev/1.0 (personal use, contact: nimmsel23@gmail.com)";
...
async function findNearestGym(lat, lng) {
  const query = `[out:json][timeout:8];(` +
    `node(around:250,${lat},${lng})["leisure"="fitness_centre"];` +
    `way(around:250,${lat},${lng})["leisure"="fitness_centre"];` +
    `node(around:250,${lat},${lng})["sport"="fitness"];` +
    `way(around:250,${lat},${lng})["sport"="fitness"];` +
    `);out center 10;`;
  const res = await fetch("https://overpass-api.de/api/interpreter", {
    method: "POST",
    headers: { "Content-Type": "text/plain", "User-Agent": GEO_USER_AGENT },
    body: query,
...
async function reverseGeocode(lat, lng) {
  ...
  const res = await fetch(url, { headers: { "User-Agent": GEO_USER_AGENT }, signal: AbortSignal.timeout(8000) });
  if (!res.ok) return null;
  const data = await res.json();
...
app.get("/fitness/geo/locate", async (c) => {
  ...
  try {
    ...
    if (gym) {
      return c.json({ ok: true, source: "gym", label: gym.name || "Fitnessstudio", mapsUrl: mapsUrlFor(gym.lat, gym.lng) });
    }
  } catch (e) { console.warn("Overpass-Lookup fehlgeschlagen:", e.message); }
  try {
    const address = await reverseGeocode(lat, lng);
    if (address) {
      return c.json({ ok: true, source: "address", label: address, mapsUrl: mapsUrlFor(lat, lng) });
    }
  } catch (e) { console.warn("Nominatim-Lookup fehlgeschlagen:", e.message); }
  return c.json({ ok: true, source: "coords", label: `${lat.toFixed(5)}, ${lng.toFixed(5)}`, mapsUrl: mapsUrlFor(lat, lng) });
});
