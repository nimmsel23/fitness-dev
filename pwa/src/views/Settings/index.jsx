import { RefreshCw, Zap, Target, User, LayoutGrid, Clock, Moon, Sun, Save, Check } from "lucide-react";
...
export default function Settings({ 
  ...
  useEffect(() => {
    setupPWAUpdate().then(manager => {
      setUpdateManager(manager);
      // Optional: Check automatically on load
      // manager?.checkForUpdate().then(available => setUpdateAvailable(available));
    });
  }, []);
  ...
  const handleCheckUpdate = async () => {
    ...
    const available = await updateManager.checkForUpdate();
       ...
       <section className="card p-8 border-accent/20 bg-gradient-to-br from-card to-bg2 shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
             <div>
                ...
                <h3 className="text-2xl font-black text-ink mb-2">PWA Lifecycle</h3>
                <p className="text-sm font-medium opacity-50 max-w-md leading-relaxed">
                   Verwalte die App-Version und stelle sicher, dass du die neuesten Features und Anatomie-Updates nutzt.
                </p>
             </div>
             
             <div className="flex flex-col gap-3 min-w-[200px]">
                {updateAvailable ? (
                   <button onClick={() => updateManager?.applyUpdate()} className="btn btn-primary py-4 px-8 font-black uppercase tracking-widest animate-pulse">
                      Update jetzt installieren
                   </button>
                ) : (
                   <button onClick={handleCheckUpdate} disabled={checkingUpdate} className="btn btn-secondary py-4 px-8 font-black uppercase tracking-widest border-line hover:border-accent">
                      {checkingUpdate ? 'Suche...' : 'Nach Updates suchen'}
                   </button>
                )}
                <div className="text-center">
                   <span className="text-[9px] font-black opacity-20 uppercase tracking-widest font-mono">
       ...
       <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <section className="card p-8 space-y-8 text-ink">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                   <label className="text-[10px] font-black uppercase tracking-widest opacity-30 mb-3 block ml-1">Zyklus Dauer (Tage)</label>
                ...
                <div>
                   <label className="text-[10px] font-black uppercase tracking-widest opacity-30 mb-3 block ml-1">Default Location</label>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
             ...
             <div className="space-y-8 bg-bg2/50 p-6 rounded-[24px] border border-dashed border-line">
                ...
                <p className="text-[9px] font-medium opacity-30 leading-relaxed text-center px-4 italic">
                   Das Interface wechselt automatisch zwischen Tag- und Nacht-Theme basierend auf deiner Systemzeit (8:00 - 20:00 Uhr).
                </p>
             </div>
          </div>
       </section>
