...
export default function Muscles() {
  ...
  const [analysis, setAnalysis] = useState({
    heavy: [], recovering: [], super: [], ready: [], scores: {}
  });

  useEffect(() => {
    setLoading(true);
    Promise.all([
      getSessionHistory(60), // Look back 60 days to find last sessions
      getAllExercises()
      ...
      for (const s of sessions) {
        ...
        if (s.activity) {
           // Cardio sets muscles to "lightly loaded" for max 72h
           if (hoursAgo <= 72) {
             ...
             for (const m of activeMuscles) {
               if (!lastSeen[m] || lastSeen[m].hours > hoursAgo) {
                 // Mark as cardio -> forces >168h category equivalent if not already heavily loaded
      ...
      // Need fake entries 1-4 to ensure react-body-highlighter maps colors correctly
      ...
      for (const m of MUSCLE_GROUPS) {
        ...
        if (!last || last.type === 'cardio' || last.hours > CAT_SUPER) {
          ready.push(m);
          scores[m] = { score: 1 }; // Index 0: Hellblau
        } else if (last.hours > CAT_RECOVERING) {
          supercomp.push(m);
          scores[m] = { score: 2 }; // Index 1: Grün
        } else if (last.hours > CAT_HEAVY) {
          recovering.push(m);
          scores[m] = { score: 3 }; // Index 2: Orange
        } else {
          heavy.push(m);
          scores[m] = { score: 4 }; // Index 3: Rot
        }
      }

      setAnalysis({ heavy, recovering, super: supercomp, ready, scores, lastSeen });
    }).catch(e => console.error(e)).finally(() => setLoading(false));
  }, []);

  const { heavy, recovering, super: supercomp, ready, scores, lastSeen } = analysis;
    ...
    <div className="pb-20">
      {loading ? (
        <div className="flex flex-col items-center justify-center py-32 opacity-30">
          ...
          <p className="text-xs font-black uppercase tracking-[0.3em]">Berechne Superkompensation…</p>
        </div>
      ) : (
